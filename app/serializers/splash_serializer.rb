class SplashSerializer < ActiveModel::Serializer
  ...
  def attributes
    super.tap { |h|
      ...
      if @options[:lineage]
        h[:lineages_count] = Splash.find_all_by_track_id(splash.track_id).size
        h[:lineages] = Splash.find_all_by_track_id(splash.track_id, :order => "created_at", :limit => 8).map{ |u| u.user }
      end
    }
  end
